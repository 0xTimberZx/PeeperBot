...
export function formatBacktest(s: BacktestSummary, trades: TradeRecord[]): string {
  ...
  return [
    ...
    "  by side (does one side carry the edge?):",
    `    UP  (buy) : n=${s.upTaken}   win=${pct(s.upWinRate)}   ROI/trade=${pct(s.upRoiPerTrade)}`,
    `    DOWN (sell): n=${s.downTaken}   win=${pct(s.downWinRate)}   ROI/trade=${pct(s.downRoiPerTrade)}`,
